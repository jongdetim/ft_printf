...
void				flaghandler_f_zeros(t_printf *data, char *str, int zeros)
{
	...
	zerostr = ft_memset(zerostr, '0', zeros); // <-- mogelijke leak door ft_memset?
	temp = str;
...
void				flaghandler2_f(t_printf *data, int len, char *str)
{
	int		zeros;
...
void				flaghandler_f(t_printf *data, int len, char *str)
{
	short	min;
